...
function openaccount() {
    ...
    if (undecouvert == "y" || undecouvert == "Y") {
        tasks.push("Ajout d'un découvert");
        ...
        tasks.push("Montant du découvert autorisé: " + decouvert + "€");
    } else {
        document.getElementById("overdraft").innerHTML = "Découvert: non autorisé";
        tasks.push("Pas de découvert autorisé");
    }
    while (sold < 500) {
        sold = Number(prompt("Saisissez le dépot"));
    }
    tasks.push("Dépot de " + sold + "€");
...
function withdraw() {
    ...
    if (check >= 0) {
        sold = sold - withdraw;
        document.getElementById("usersold").innerHTML = "Solde (€): " + sold;
        tasks.push("Retrait de " + withdraw + "€");
        tasks.push("Solde restant: " + sold + "€");
    }
    else {
    ...
    if (sold < 0) {
        ...
        tasks.push("Vous êtes à découvert");
    }
    readtab();
}
...
function agios() {
    ...
    tasks.push("Sur " + day + " Jour(s) vous avez " + result.toFixed(2) + "€ d'intérêts");
...
function readtab() {
    ...
    for (let i = 0; i < tasks.length; i++) {
        ...
        document.getElementById("tasks-show").innerHTML = mytask;
    }
    console.log(tasks);
}